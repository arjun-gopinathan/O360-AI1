...
import com.Pages.Methods.Methods_TicketOnBehalf;
...
public class TC_TicketOnBehalf extends Base_Class{
	...
	Methods_TicketOnBehalf methods_TicketOnBehalf;
	...
	public void reference() throws InterruptedException {
		...
		methods_TicketOnBehalf =new Methods_TicketOnBehalf();
		log = new Log();		
		screenshot = new com.Utility.ScreenShot(driver);
		ExcelReader = new com.Utility.ExcelReader("TicketsOnBehalf");		
	}
	@BeforeMethod
	...
	public void runAlllScenarios(Map<Object, Object> testdata, ITestContext context)
		...
		try {
			...
			String employee=testdata.get("Employee Name").toString();
			...
			if (testdata.get("Run").toString().equalsIgnoreCase("Yes")) {
				...
				Base_Class.SetUp("TicketsOnBehalf");
				...
				ExtentTestManager.startTest("TC -2 Navigate to Ticket Menu and Ticket on behalf page");
				methods_TicketOnBehalf.clickandNavigateToTicketOnBehalf();
				
				ExtentTestManager.startTest("TC -3 Validating tickets on behalf page is displayed or not");
				methods_TicketOnBehalf.verifyTickteOnBehalfPage();
				
				ExtentTestManager.startTest("TC -4 Enter employee name On behalf tciket needs to be created");
				methods_TicketOnBehalf.serachAndSelectEmployee(employee);
			
							
				ExtentTestManager.startTest("TC- 5 Enter the subject field");
				methods_TicketOnBehalf.enterSubject(subject);
				
				ExtentTestManager.startTest("TC- 6 Select the category dropdown");
				methods_TicketOnBehalf.selectCategory(category);
				
				ExtentTestManager.startTest("TC- 7 Select the ticket type");
				methods_TicketOnBehalf.selectTicketType(ticketType);
				
				ExtentTestManager.startTest("TC- 8 Select the priority type");
				methods_TicketOnBehalf.selectPriorityType(priorityType);
				
				ExtentTestManager.startTest("TC- 9 Verify status field is disabled");
				methods_TicketOnBehalf.areFieldsNonEditable();
											
				ExtentTestManager.startTest("TC- 10 select the wathchers");
				methods_TicketOnBehalf.addWatchers(watcher); 
				
				ExtentTestManager.startTest("TC- 11 Enter the description");
				methods_TicketOnBehalf.enterDescription(description);				
				
				ExtentTestManager.startTest("TC- 12 Upload the file");
				...
				methods_TicketOnBehalf.uploadFileTicket(browseFilePath);
				//methods_CreateTicket.uploadFileTicket("FilePath");  
							
				ExtentTestManager.startTest("TC- 13 Submit the ticket");
				methods_TicketOnBehalf.submitButton();
				
				ExtentTestManager.startTest("TC- 14 Validate Ticket confiramtion dialog appears");
				methods_TicketOnBehalf.validateConfirmationPopUpDisplayed();
				
				ExtentTestManager.startTest("TC- 15 Confirmation message");
				methods_TicketOnBehalf.confirmationMessageDisplayed();
				methods_TicketOnBehalf.confirmationMessage();
				
				ExtentTestManager.startTest("TC- 16 Handle Desktop notification");
				Thread.sleep(2000);
				handlePopupDesktop();
				
						   
				ExtentTestManager.startTest("TC -17 Navigate to Ticket Menu and My Tickets page");
				methods_MyTicketsPage.navigateMyTickets();
				
				ExtentTestManager.startTest("TC - 18 Validating my tickets page is displayed or not");
				methods_MyTicketsPage.verifyMyTicketsPage();
								
				ExtentTestManager.startTest("TC- 19 Validate ticket status and ticket open");
				methods_MyTicketsPage.valiadteTicketStatusandTickets(ticketStatus, ticket);
				
				ExtentTestManager.startTest("TC- 20 Validate downlaod Tickets");
				methods_MyTicketsPage.downloadTickets();
				
				ExtentTestManager.startTest("TC- 21 Verify Default Search Field Selection");
				...
				ExtentTestManager.startTest("TC- 22  Search By Tickets");
