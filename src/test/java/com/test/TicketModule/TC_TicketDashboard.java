...
public class TC_TicketDashboard extends Base_Class{
	...
	public void reference() throws InterruptedException {
		...
		ExcelReader = new com.Utility.ExcelReader("TicketDashboard");		
	}
	@BeforeMethod
	...
	public void runAlllScenarios(Map<Object, Object> testdata, ITestContext context)
		...
		try {
			...
			String subCategory = testdata.get("Sub Category").toString();
			String assignto = testdata.get("Assign To").toString();		
			String rerouteCategory = testdata.get("Reroute Category").toString();
			String subType = testdata.get("Sub Type").toString();
		...
		//	String watcher2 = testdata.get("Watcher2").toString();
			...
			if (testdata.get("Run").toString().equalsIgnoreCase("Yes")) {
				...
				Base_Class.SetUp("TicketDashboard");
				...
				methods_TicketDashboard.confirmationMessageDisplayed();
				...
				methods_TicketDashboard.navigateTicketDashboard();
				
				ExtentTestManager.startTest("TC -3 Valiadte Dashboard page successfully landed in");
				methods_TicketDashboard.valiadteTicketDashboardPage();
														
				ExtentTestManager.startTest("TC- 4 Select the Category");
				methods_TicketDashboard.selectCategory(category);
				
				ExtentTestManager.startTest("TC- 5 Select the ticket type created on");
				methods_TicketDashboard.selectTicketCreatedOn(ticketTypeCreatedOn);
				
				ExtentTestManager.startTest("TC- 6 Search the ticket number");
				methods_TicketDashboard.searchTicketNumber(ticketNO);
											
				ExtentTestManager.startTest("TC- 8 Click the search icon");
				methods_TicketDashboard.clickSearchIcon();
				
				ExtentTestManager.startTest("TC- 9 Verify searched ticket number displayed");
				methods_TicketDashboard.verifysearchedTicketNumberDisplayed(ticketNO);	
				
				ExtentTestManager.startTest("TC- 10 Click on searched ticket");
				methods_TicketDashboard.clickOnSearchedTicket();
					
				ExtentTestManager.startTest("TC- 11 Verify ticket deatils displayed");
				methods_TicketDashboard.verifyTicketdetails();	
				
				ExtentTestManager.startTest("TC- 12 Fields are non editable");
				methods_TicketDashboard.areFieldsNonEditable();
						
				ExtentTestManager.startTest("TC- 13 Select the priority type");
				methods_TicketDashboard.selectPriorityType(priorityType);
				
				ExtentTestManager.startTest("TC- 14 Select the status");
				methods_TicketDashboard.selectStatus(status);
				
				ExtentTestManager.startTest("TC- 15 Select the Subcategory");
				methods_TicketDashboard.selectSubCategory(subCategory);
				
				ExtentTestManager.startTest("TC- 16 Select the reroute category" );
				methods_TicketDashboard.SelectRerouteCategory(rerouteCategory, subType);
				
				ExtentTestManager.startTest("TC- 17 Click on Audit trail");
				methods_TicketDashboard.clickAuditTrail();
						
				ExtentTestManager.startTest("TC- 18 Validate Audit trail" );
				methods_TicketDashboard.valiadteAuditTrail();
				
				ExtentTestManager.startTest("TC- 19 Close on Audit trail");
				methods_TicketDashboard.close();
				
				ExtentTestManager.startTest("TC- 21 validate the ticketHistory");
				methods_TicketDashboard.ticketHistory();
				methods_TicketDashboard.validateTicketHistory();
				methods_TicketDashboard.closeDialog();
				
				ExtentTestManager.startTest("TC- 22 Validate ticket status history" );
				methods_TicketDashboard.ticketStatusHistory();
				methods_TicketDashboard.validateTicketStatusHistory();
				methods_TicketDashboard.closeDialog();
