...
import com.Pages.Methods.Methods_TicketDashboard;
...
public class TC_AssignedTickets extends Base_Class {
	Methods_TicketDashboard methods_TicketDashboard;
	...
	public void reference() throws InterruptedException {
		...
		methods_TicketDashboard = new Methods_TicketDashboard();
		...
		ExcelReader = new com.Utility.ExcelReader("AssignedTickets");		
	}
	...
	public void runAlllScenarios(Map<Object, Object> testdata, ITestContext context)
		...
		//	String ticketNO = testdata.get("Ticket Number").toString();
			...
			String assignto = testdata.get("Assign To").toString();		
			...
			String category1 = testdata.get("Category").toString();
			String ticketType = testdata.get("Ticket Type").toString();
			String priorityType1 = testdata.get("Priority Type").toString();
		...
		//	String watcher2 = testdata.get("Watcher2").toString();
			String description = testdata.get("Add Description").toString();
				...
				Base_Class.SetUp("AssignedTickets");
				...
				methods_CreateTicket.selectCategory(category1);
				...
				methods_CreateTicket.selectPriorityType(priorityType1);
				...
				methods_AssignedTickets.confirmationMessageDisplayed();
				...
				ExtentTestManager.startTest("TC -2 Navigate to Ticket Menu and land into Dahboard Page");
				methods_AssignedTickets.navigateTicketDashboard();
				
				ExtentTestManager.startTest("TC -3 Valiadte Dashboard page successfully landed in");
				methods_AssignedTickets.valiadteTicketDashboardPage();
														
				ExtentTestManager.startTest("TC- 4 Select the Category");
				methods_AssignedTickets.selectCategory(category);
				
				ExtentTestManager.startTest("TC- 5 Select the ticket type created on");
				methods_AssignedTickets.selectTicketCreatedOn(ticketTypeCreatedOn);
				
				ExtentTestManager.startTest("TC- 6 Search the ticket number");
				methods_AssignedTickets.searchTicketNumber(ticketId);
											
				ExtentTestManager.startTest("TC- 7 Click the search icon");
				methods_AssignedTickets.clickSearchIcon();
				
				ExtentTestManager.startTest("TC- 8 Verify searched ticket number displayed");
				methods_AssignedTickets.verifysearchedTicketNumberDisplayed(ticketId);	
				
				ExtentTestManager.startTest("TC- 9 Click on searched ticket");
				methods_AssignedTickets.clickOnSearchedTicket();
					
				ExtentTestManager.startTest("TC- 10 Verify ticket deatils displayed");
				methods_AssignedTickets.verifyTicketdetails();	
				
				ExtentTestManager.startTest("TC- 11 Fields are non editable");
				methods_AssignedTickets.areFieldsNonEditable();
						
				ExtentTestManager.startTest("TC- 12 Select the priority type");
				methods_AssignedTickets.selectPriorityType(priorityType);
				
				ExtentTestManager.startTest("TC- 13 Select the status");
				methods_AssignedTickets.selectStatus(status);
				
				ExtentTestManager.startTest("TC- 14 Assign to concerned employee ");
				methods_AssignedTickets.selectAssignTo(assignto);
								
				ExtentTestManager.startTest("TC- 15 Click on Audit trail");
				methods_AssignedTickets.clickAuditTrail();
						
				ExtentTestManager.startTest("TC- 16 Validate Audit trail" );
				methods_AssignedTickets.valiadteAuditTrail();
				
				ExtentTestManager.startTest("TC- 17 Close on Audit trail");
				methods_AssignedTickets.close();
				
				ExtentTestManager.startTest("TC- 18 validate the ticketHistory");
				methods_AssignedTickets.ticketHistory();
				methods_AssignedTickets.validateTicketHistory();
				methods_AssignedTickets.closeDialog();
				
				ExtentTestManager.startTest("TC- 19 Validate ticket status history" );
				methods_AssignedTickets.ticketStatusHistory();
				methods_AssignedTickets.validateTicketStatusHistory();
				methods_AssignedTickets.closeDialog();
				
				ExtentTestManager.startTest("TC- 20 Submit the ticket ");
				methods_AssignedTickets.submitTheTicket();
				
				ExtentTestManager.startTest("TC- 21 Close the ticket");
				methods_AssignedTickets.close();
				
				ExtentTestManager.startTest("TC- 21 Signout " );
				Signout();
				
				ExtentTestManager.startTest("TC- 22 Login with assigned User" );
				methods_AssignedTickets.assignedUser(usernameAssigned);
				
				
				ExtentTestManager.startTest("TC- 23 Handling Covid popup and Desktop notifications");
				handlePopupCovid();
				Thread.sleep(1000);
				handlePopupDesktop();
				Thread.sleep(2000);
				
				ExtentTestManager.startTest("TC- 24 Navigate to tickets menu and assigned tickets" );
				methods_AssignedTickets.navigateToAssignedTicketsMenu();
				
				ExtentTestManager.startTest("TC- 25 Validate assigned tickets page landed successfully" );
				methods_AssignedTickets.valiadteAssignedTicketPage();
				
				ExtentTestManager.startTest("TC- 26 Download all assigned tickets" );
				methods_AssignedTickets.downloadTickets();
				
				ExtentTestManager.startTest("TC- 27 Verify Default Search Field Selection");
				...
				ExtentTestManager.startTest("TC- 28  Search By Tickets");
				methods_AssignedTickets.searchTicket(ticketId);
				methods_AssignedTickets.clickSearchButton();
				methods_AssignedTickets.verifysearchedTicketNumberDisplayed1(ticketId);
				
				ExtentTestManager.startTest("TC- 29 Access searched ticket page");
				methods_AssignedTickets.clickOnSearchedTicket();
				methods_AssignedTickets.verifyTicketdetails();
				
				ExtentTestManager.startTest("TC- 30 Verify non-editable fields");
				methods_AssignedTickets.areFieldsNonEditableAssignedTickets();
				
				ExtentTestManager.startTest("TC- 31 Change the status ");
				methods_AssignedTickets.selectStatus(changeStatus);
				
				ExtentTestManager.startTest("TC- 32 Submit the ticket ");
				methods_AssignedTickets.submitTheTicket();	
				
				ExtentTestManager.startTest("TC- 33 Add comments for ticket ");
				methods_AssignedTickets.enterAddComments(addComments);
				
				ExtentTestManager.startTest("TC- 34 Change the status ");
				methods_AssignedTickets.selectStatus(changeStatus1);
				
				ExtentTestManager.startTest("TC- 35 Submit the ticket ");
				methods_AssignedTickets.submitTheTicket();
				
				ExtentTestManager.startTest("TC- 36 Add resolved comments");
				methods_AssignedTickets.resolvedAddComments(resolveComments);
					
				ExtentTestManager.startTest("TC- 37 Close the ticket");
				methods_AssignedTickets.close();
				methods_AssignedTickets.close();
